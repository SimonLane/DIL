...
# compatible with firmware v3.1 (matchbox)

# =============================================================================
# Z-Stack SCRIPT
+ MATCHBOX laser integration
+ check for DIL firmware compatability
...
+ Timelapse
+ Multi-position
+ background correction, inc. hot pixel removal
+ display center image of each stack in console, w/ lookup table
- chamber and room temp logging - hardware not working!!
# =============================================================================
"""
...
#(1004.726, 958.408, 2924.494), # W1_1_D_LARGE_mt
#(872.830, 1005.400, 2922.312),  # W1_2_SMALL_MT
#(1063.022, 1709.146, 2979.154), # W2_SMALL_MT
#(941.206, 3607.110, 2946.184),  # W4_good_MT
#(1028.552, 4616.972, 2963.542), # W5_LARGE_MT
#(1083.334, 5597.912, 2929.202), # W6_GOOD_MT
#(1067.992, 6559.256, 2914.632) # W7_TINY_MT

   

   
##(987, -2065, 2928),    #_BL_G       f0
##(745, -1005, 2935),     #_BR_G      f1
#(994, -368, 2903),     #_TR_Tiny    h0
#(1007, 971, 2921),      #_BL_G      XX
#(822, 1606, 2944),      #_T_G       h1
#(795, 3918, 2919),      #_BR_G      XX
##(1091, 4922, 3065),     #_BL_L      f2
##(861, 6599, 3068),      #_TR_Beaut  f3
#(1067, 7892, 3157),     #_B_S       h3
#(896, 8759, 2977)       #_R_Beaut   h2


(1117, -3350, 2912), #P0 50
(1161, -2354, 2914), #P1 100 
(1016, -1250, 2924), #P2 200
(1252, -356, 2946), #P3 300
(1176, 656, 2959), #P4 400 
(1273, 1907, 2937), #P5 500
(1101, 2790, 2945), #P6 600 
(1054, 3669, 2968) #P7 700
   ]
...
_405        =  [0,         100,        200,         'DAPI-1P',            405,         1]
_488        =  [1,         100,         200,         'autofluorescence_1P',           488,         2]
_520        =  [1,         20,         10,         'scatter',         520,         5]
_638        =  [0,         100,        30,         '638_HRP',                    638,         6]
_MaiTai1    =  [0,         10,         100,         'NADH',            730,         4]
_MaiTai2    =  [0,         10,         100,         'FAD',               875,         5]
_scatter    =  [0,         10,          10,          'scatter',                 488,         6]

lasers = [_405,_488,_520,_638,_MaiTai1,_MaiTai2,_scatter] # change order here to change channel order

nZ          = 10       # Number of slices
sZ          = 10.0    # slice separation (micrometers)


# name        = "AH_06_Spheroid_SHrep_xyzt"          # experiment name
name        = "Timeout debug test"          # experiment name
...
def stage_stable(axis): #tests stage to make sure movement is complete
    ...
    if(Status == "9"): 
        ...
        Error = GO.readline().decode(codec)
        print('Stage Error code:', Error)
    return False   # stage moving, or error
...
def display_frame(frame, p, channel, t, z):
    plt.figure()
    im = plt.imshow(frame, cmap='viridis')
    plt.colorbar(im)
    plt.title("T:{}, P:{}, C:{}, Z:{}".format(t, p, channel[1], z))
    plt.show()
# =============================================================================
# FILE HANDLING
# =============================================================================
...
def estimate_laser_power(wav, percent):
    if wav>700: return 0.0, 0.0 # can't lookup power for MaiTai, return zeros
...
if multi_photon:
    ...
    try:
        ...
        con_MaiTai = True
        print("connected to MaiTai serial") 
        
        log_append("connected to MaiTai serial")
        time.sleep(0.05)
        close_shutter()
...
try:
    ...
    for channel in channels: #setting laser emmissions for selected channels
        if channel[2] < 700: # filter, visible wavelengths only
...
# =============================================================================
if bg_images:
    # create folders for background images
    os.makedirs("{}/background".format(folder))
        
    DIL.write(bytes("/musical.1;\r", codec))                        # set to musical mode (no stage movement required)
    for channel in channels:
        ...
        CAM.setup_acquisition(mode="sequence", nframes = n_bg_images)

        # start acqusition        
        CAM.start_acquisition() # Option 1 - testing if acquisition start position affects multi-pos timelapse 
        time.sleep(0.1)        
        DIL.write(bytes("/stop;\r" , codec))
        DIL.write(bytes("/stack.%s.%s;\r" %(int(channel[4]),n_bg_images), codec))
...
# =============================================================================
        ...
        # stage to start position. Perform z-stack centered around current position
            ...
            clear_stage_buffer()
            while(True): 
                if(time.time() > tstart + stage_timeout):
...
# =============================================================================
        ...
        # report timing stats    
            ...
            DIL.write(bytes("/stop;\r" , codec))            # TTL off the visible lasers and park the laser
